...
function assignThirdsToSlots(
  ...
  function backtrack(remaining: Slot[]): boolean {
    ...
    const scored = remaining
      ...
      .sort((a, b) => a.cands.length - b.cands.length || a.slot.label.localeCompare(b.slot.label));
      ...
      if (!assignment.has(label)) {
        ...
        if (rankedThirds.length === 8) {
          ...
          console.warn(
            ...
            'Qualifying thirds:',
            rankedThirds.map((t) => `${t.team.countryCode}(${t.groupLetter})`).join(', '),
          );
        }
      }
    }
 ...
 */
export function validateR32Uniqueness(bracketMatches: BracketMatchData[]): string[] {
...
export function buildBracket(
  ...
  const thirdSlotLabels = [
    ...new Set(
      sorted.flatMap((m) => [m.homeSlot, m.awaySlot]).filter(
        (s): s is string => !!s && s.startsWith('3') && s.length > 2,
      ),
    ),
  ];
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    for (let rank = 1; rank <= 8; rank++) {
      const team = slotMap.get(`T${rank}`);
      if (!team) { thirds.push(`T${rank}=?`); continue; }
      const groupEntry = [...slotMap.entries()].find(
  ...
  for (const match of sorted) {
    const home = resolveSlot(match.homeSlot ?? '', slotMap, byMatchNum, thirdSlots);
    const away = resolveSlot(match.awaySlot ?? '', slotMap, byMatchNum, thirdSlots);
